...
//        }
    ...
    public Result<IPage<House>> recommendHouses(@RequestBody RecommendParam param) {
        // 校验必传参数
        if (param.getStatus() == null || param.getCurrentPage() == null || param.getSize() == null) {
            return Result.error("参数不能为空（status、currentPage、size为必传）");
        }
        IPage<House> housePage = houseService.recommendHouses(param);
